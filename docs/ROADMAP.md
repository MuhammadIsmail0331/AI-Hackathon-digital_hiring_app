...
| 2026-09-04 | **Correctness + Heritage-Craft pass** — (1) OTP mandatory end-to-end: register/reset schemas require the 6-digit code, server-side bypass removed, "Continue without OTP" UI path deleted. (2) Input hardening: per-skill 60-char caps, control-char stripping (Urdu/emoji safe) on free text, PUT /api/employer/jobs/[id] now Zod-validated (was raw-body write). (3) matchReason stored as structured JSON, rendered via `renderMatchReason()` in the active locale (legacy rows render as-is). (4) Job edit UI on detail page (title/wage/workers/description; OPEN/DRAFT only). (5) 3-step icon indicator on create-job; silent catches → ErrorBanner retry states on 5 worker pages. (6) Visual identity: LayeredBackground (aurora canvas + dot-grid, z-safe), HeritageFrame border system with corner rivets, BrandAccent truck-art ribbon v2, hero aurora, panel-lift cards; old doodles/garland retired. Build green. |
...
| 2026-09-07 | **Post-Regionals Round 1** � Photo uploads (avatar + worker portfolio), in-app chat + AI support bot (bilingual rule-based fallback, Gemini-ready), Leaflet map location picker, global AutoNav, login auto-redirect, VoiceHelp Urdu TTS, scroll-to-invalid-field |

| 2026-09-08 | **Post-Regionals Round 2** — Chat and My-Photos entries added to the mobile bottom nav (both roles) + Nav i18n keys (EN/UR). `/worker/media` hosts avatar & portfolio uploads; `/chat` hosts direct threads + AI support bot. Still queued: LocationMap section inside media page, photo attach in job form, portfolio display on public profiles. |
