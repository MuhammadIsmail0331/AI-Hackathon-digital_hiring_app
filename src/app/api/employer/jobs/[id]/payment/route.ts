 ...
 *   { action: "SECURE" }  â†’ employer secures the total wage amount (SECURED)
 *   { action: "RELEASE" } â†’ employer releases the held payment after job completion (RELEASED)
 */
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    // â”€â”€ SECURE: lock in the total wage before/during the job â”€â”€
    if (action === "SECURE") {
    ...
    // â”€â”€ RELEASE: pay out after the job is completed â”€â”€
    ...
    const platformFee = Math.round(job.wage * job.numberOfWorkers * 0.05);
    const workerPayout = job.wage * job.numberOfWorkers - platformFee;
    ...
    for (const offer of acceptedOffers) {
      await createNotification(
        ...
        "SYSTEM",
        "Payment Released",
        `The employer has released ${workerPayout.toLocaleString()} PKR for the job "${job.title}". Payment settled.`,
        { jobId: job.id, link: `/worker/my-jobs/${job.id}` }
      );
    }
