    ...
    async function fetchProfile() {
      try {
        ...
        if (data.profile) {
          ...
          setAvailableDays(
            Array.isArray(p.availableDays) ? p.availableDays : []
          );
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-6 pb-24 sm:pb-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">{t("completeProfileDesc")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            {skills.length > 0 && (
              <p className="mt-2 text-xs text-gray-500">{skills.length} {common("selected")}</p>
            )}
          </section>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <p className="mb-3 text-sm text-gray-500">{t("selectExperience")}</p>
              ...
              {EXPERIENCE_LEVELS.map((level) => {
                ...
                return (
                  <button key={level.id} type="button" onClick={() => selectExperienceLevel(level.id)}
                    ...
                    <div className={cn("text-sm font-semibold", isSelected ? "text-blue-700" : "text-gray-700")}>{t(level.id)}</div>
                    <div className={cn("mt-1 text-xs", isSelected ? "text-blue-500" : "text-gray-500")}>{level.years} {t("yearsExp")}</div>
                  </button>
                );
              })}
            </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-700">4</span>
              {t("location")}<span className="text-xs text-red-500">*</span>
            </h2>
            <p className="mb-3 text-sm text-gray-500">{t("selectCity")}</p>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-700">5</span>
              {t("availability")}
            </h2>
            <button type="button" onClick={() => setIsAvailable(!isAvailable)}
              className={cn("mb-4 flex w-full items-center justify-between rounded-xl border-2 p-4 transition",
                isAvailable ? "border-green-400 bg-green-50" : "border-gray-200 bg-gray-50")}>
              <div className="flex items-center gap-3">
                <div className={cn("flex h-10 w-10 items-center justify-center rounded-full",
                  isAvailable ? "bg-green-100" : "bg-gray-200")}>
                  {isAvailable ? (
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-green-600"><path d="M20 6 9 17l-5-5" /></svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-500"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
                  )}
                </div>
                <span className={cn("text-sm font-semibold", isAvailable ? "text-green-700" : "text-gray-600")}>
                  {isAvailable ? t("availableNow") : t("notAvailable")}
                </span>
              </div>
              <div className={cn("relative h-7 w-12 rounded-full transition", isAvailable ? "bg-green-500" : "bg-gray-300")}>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <input type="number" value={expectedWage} onChange={(e) => setExpectedWage(e.target.value)}
                ...
                className="w-full rounded-xl border border-gray-300 px-4 py-3 text-lg font-semibold outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
              <div className="flex h-12 shrink-0 items-center rounded-xl bg-blue-100 px-4 text-sm font-bold text-blue-700">
                {t("pkr")}<span className="ms-1 text-xs font-normal">{common("perDay")}</span>
              </div>
            </div>
            <p className="mt-2 text-xs text-gray-500">{t("wageHint")}</p>
          </section>

          {/* ── 7: Bio ── */}
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 text-xs font-bold text-gray-500">7</span>
              {t("bio")}<span className="text-xs text-gray-500">({common("optional")})</span>
            </h2>
            ...
            <p className="mt-2 text-xs text-gray-500">{bio.length}/500 · {t("bioHint")}</p>
          </section>
